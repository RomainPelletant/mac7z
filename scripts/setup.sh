...
if [[ "$(uname -s)" != "Darwin" ]]; then
  warn "This script download the bundled 7zz binary for macOS only"
  warn "On Linux, 7zip is installed via : sudo apt install p7zip-full"
  exit 0
fi
...
# ── Download ────────────────────────────────────────────────────────────
step "Download 7-Zip ${SEVENZIP_VERSION} (macOS universal)..."
...
# Binary name is 7zz in macOS archive
